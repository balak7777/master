...
class DistrictwisePercentage extends React.Component {
   constructor(props) {
   ...
   renderTableData() {
      return this.state.data.map((data, index) => {
         ...
         const { district, recoverper, deathper} = data //destructuring
         return (
            <tr key={district}>
   ...
   formStructureDistrict(data) {
      ...
      for (var item in data) {
         ...
         if (data.hasOwnProperty(item)) {
            ...
            data[item].recoverper = ((data[item].recovered*100)/data[item].confirmed).toFixed(2)+'%';
            data[item].deathper = ((data[item].deceased*100)/data[item].confirmed).toFixed(2)+'%';
...
export default DistrictwisePercentage